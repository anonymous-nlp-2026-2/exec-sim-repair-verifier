...
\noindent\textbf{Appendix overview.} A: Oracle resolve advantage. B: Ablation studies. C-F: Extended analysis (error complementarity, parseable accuracy, forced parsing, qualitative errors, failed interventions, mutation scaling, inference efficiency, re-repair, chain-of-thought). G: TOST sensitivity (G.1: Bayesian ROPE). H: Effect sizes. I: Holm-Bonferroni. J: Mixed-effects regression. K: Parseable bias. L: CWM convergence. M: Few-shot. N: Threats. O: Downsample. P: Use of AI assistants.
...
We tested whether per-test diagnostic feedback improves downstream repair. A Qwen3-8B repair agent received either per-test diagnostic signals (which specific tests failed and why) or binary accept/reject feedback, evaluated on 44 SWE-bench Verified instances. The diagnostic condition achieved accuracy no higher than the binary baseline ($A_1 = 52.3\%$, $p = 0.42$, effect size $\approx 0$) across all $n{=}44$ evaluated instances; the $n{=}14$ failing instances (where diagnostic feedback could potentially change the outcome) showed no differential benefit. We found no evidence that per-test diagnostic information translates to repair improvements at this scale, though the analysis is underpowered (estimated power $\approx 0.3$ for a medium effect size at $n{=}44$). The negative result may reflect insufficient agent capability to exploit fine-grained feedback, insufficient sample size, or a genuine limitation of the diagnostic-to-repair translation.
